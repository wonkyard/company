...
- **Missing tests**: check whether the engineering report includes a test result (flag as Medium if missing)
...
- On BLOCK, never run push or commit. Report the rejection reason to the Chief of Staff.
- On PASS, you're only approving that `git add`/`commit`/`push` may proceed — actually running those commands is the Chief of Staff's or `engineering`'s job, not yours.
