...
For Codex sessions, the project-scoped equivalents live in `.codex/agents/*.toml`.
Use `gpt-5.6-luna` for bounded mechanical research, summaries, support, and portfolio/repo bookkeeping;
`gpt-5.6-terra` for implementation and repo-team builds; `gpt-5.6-sol` for market, financial, and
public-facing judgment; and `gpt-6-astra` for independent security and release Gates. The explicit role file
wins over the `[agents]` default. Do not overwrite the Claude frontmatter when changing Codex routing, and
report the observed runtime model separately from the configured/default model.
